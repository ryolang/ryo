...
* **Languages compared:** Rust, Swift, Go, Python, and Ryo (AOT vs JIT).
* **Highlights:** Ryo AOT runs **23.9x faster than Python** (98.4 ms vs 2,350 ms) with ~3x less memory and the lightest RSS after Rust (8.52 MB), but trails Rust by 19.7x — far wider than byte_slicing's 1.7x on a flat scan, and behind Go (41.0 ms) and Swift (58.2 ms) too. The deep per-byte call tree (no Cranelift inliner) plus §18 checked-arithmetic guards on every position update compound where a flat loop amortized them (tracked as I-185; guard fusion is I-165).
