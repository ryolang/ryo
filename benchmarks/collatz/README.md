...
2. Every checked op materializes the overflow flag into a boolean and branches on it later (`adds` + `cset vs` + `tst` + `b.ne`, 4 instructions where Swift emits `adds` + `b.vs`). Three checked ops per iteration (`3 * n`, `+ 1`, `steps += 1`) cost +6 instructions — the known flag-fusion gap, I-165.
