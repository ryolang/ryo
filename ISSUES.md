...
**Summary:** Recursion past the OS stack limit in a Ryo AOT binary hits the guard page blind and dies with SIGSEGV (exit 139) and no message; Rust under identical conditions aborts cleanly with `thread 'main' has overflowed its stack` (exit 134). Reproduced 2026-09-15 with `benchmarks/eager_destruction/eager_destruction.ryo` at depth 210,000 (8176 KB main-thread stack, macOS): Ryo segfaults at ~208k–210k frames while both Rust arms abort with the diagnostic at ~74.6k. Recursion-heavy programs (parsers, tree walkers) get an undebuggable crash instead of an error.
**Resolution:** Emit a stack-limit check in function prologues (compare SP against a limit recorded at startup, abort with a message) or install a SIGSEGV/SIGBUS handler on an alternate signal stack that recognizes guard-page hits and reports them; Cranelift provides no stack probes for this, so the check or handler is ours. Decide the budget semantics (fixed limit vs querying the main-thread stack size at startup) and add a test that recurses past the limit and asserts a clean exit code plus diagnostic.

### I-178 — Tail-position calls are not emitted as tail calls; recursion depth is frame-size-limited

**Files:** `ryo-backend/src/codegen/expr.rs` (`emit_call`), `ryo-backend/src/codegen/mod.rs` (`emit_stmt` tail handling), `ryo-frontend/src/ownership/` (pending-drop information at tail position)
...
### I-186 — No zero-copy `str → bytesview` projection; byte scanning forces an owned `to_bytes()` copy

**Files:** `ryo-frontend/src/sema/expr.rs` (bridge-method family, ~line 291), `ryo-frontend/src/ownership/views.rs` (`root_owner` registration), `ryo-backend/src/codegen/expr.rs` (bridge-method lowering), spec §4.4, `landing/reference/index.html`
**Summary:** `strview` has no integer indexing (only slice-compare), so byte-level scanning of text — the parser shape — must go through `bytes`, and the only way to get `bytes` from a `str` today is `to_bytes()`: an owned copy (`__ryo_str_to_bytes` allocates + memcpy's). In `benchmarks/json_validate` this costs one full extra document buffer at the RSS peak (8.52 MB vs Rust's 5.91 MB, whose `as_bytes()` borrows the `String` zero-copy); every parser-style workload pays the same copy. A `str.as_bytes() -> bytesview` builtin would project the string's buffer read-only — same `(ptr, len)` repackaging a slice performs, no allocation.
**Resolution:** Add `as_bytes` to the bridge-method family (receiver `str`/`strview` → `bytesview`), register the call result in `root_owner` as a view rooted at the receiver (freeze + P4 last-use lifting come free — the consuming machinery is generic; the new piece is a call-result-as-view creation path, today only slices create views), and lower it in codegen as a pure `(ptr, len)` repackage with no runtime call. Sized at ~150–250 lines including sema/ownership tests (view can't escape, can't mutate during view, freeze lifts at last use), one integration test, a spec §4.4 line, and a reference-page entry. Composes with the retaining-views spec work: `shared[str].as_bytes()` is the retaining variant that may escape the call. Switch `benchmarks/json_validate` to it and re-measure RSS.

### I-187 — Lint: `to_bytes()` result that never mutates nor escapes should be `as_bytes()`

**Files:** `ryo-frontend/src/ownership/` (use classification + liveness already computed), `ryo-core/src/diag.rs` (warning precedent: W0003)
**Summary:** Once `as_bytes()` exists, a `to_bytes()` call whose result is only ever read is a silent O(n) allocation + copy for no benefit — a bug-shaped smell in exactly the hot paths where it hurts most (the `benchmarks/json_validate` RSS story). The compiler already has everything needed to catch it: `to_bytes()` calls are identifiable in TIR (the `__ryo_str_to_bytes` bridge callee), and the ownership pass classifies every use of every binding (read / immutable borrow / move / `inout`) with branch merging and last-use. A value born from `to_bytes()` whose uses are exclusively reads and immutable borrows — which per Rules 5 and 6 cannot escape the caller — and which is never returned, moved, stored, or mutated (`bytes_push`, `inout`) is provably replaceable by `as_bytes()`.
**Resolution:** Post-ownership-pass lint emitting a warning (new W-number after W0003) with a machine-applicable-style suggestion: "this `bytes` is never mutated and never escapes — use `as_bytes()` (zero-copy view) instead of `to_bytes()`". Follow moves across local bindings when classifying; treat `move` parameters, returns, aggregate stores, `bytes_push`/`inout` as legitimate. First expected hit: `benchmarks/json_validate` itself — its `to_bytes()` copy is never mutated nor returned, so the lint should flag it there before the explicit switch to `as_bytes()` lands.
