...
**Files:** `ryo-frontend/src/builtins.rs`, `ryo-frontend/src/sema/builtins.rs` (`emit_builtin_call`), `ryo-backend/src/codegen/expr.rs` (print emission)
...
### I-156 — Heap-`str` last use inside a loop slice-comparison or inline `assert` miscompiles

**Files:** `ryo-frontend/src/ownership.rs`, `ryo-backend/src/codegen/expr.rs`
**Summary:** Two repro shapes break when a heap-owned `str`'s last use participates in certain expressions (found while authoring the Phase 0 benchmarks; both repros verified at d6cf89a). (a) Scanning a heap-built string with a slice comparison inside a loop —
`fn main():` / `mut s: str = ""` / `for i in range(0, 8): str_push(&s, "fox ")` / `mut i = 0` / `while i + 3 <= s.len():` / `if s[i:i+3] == "fox": …` / `i += 1`
...
**Summary:** `include_bytes!` bakes the full staticlib into the compiler binary. The archive is `no_std` since I-043 (std, and the `_Unwind_*` link wart with it, is gone), but it still bundles all of core's precompiled objects, which is what keeps it large. Measured 2026-08-24 (aarch64-apple-darwin): 6.06 MB debug / 5.81 MB release.
...
**Resolution:** Gate the debug sections behind a `--verbose` flag, then migrate tests to exit-code assertions. The harness migration (formerly I-098) has already landed — `run_file` itself is the remaining work.
...
- Architecture analysis: [docs/dev/architecture_analysis.md](docs/dev/architecture_analysis.md), refreshed at [docs/dev/architecture_analysis_2026_08_20.md](docs/dev/architecture_analysis_2026_08_20.md) (source of I-131–I-137) and [docs/dev/architecture_analysis_2026_08_24.md](docs/dev/architecture_analysis_2026_08_24.md)
