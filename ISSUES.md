...
### I-143 — Flat AST diverges from Zig's `Ast.zig` on spans and index conventions

**Files:** `ryo-core/src/ast.rs` (`Ast.spans`, `NodeRef`), `ryo-frontend/src/parser.rs`, `ryo-core/src/ast_pretty.rs`
**Summary:** The I-126 rewrite cloned Zig's node layout (`tag` + `data` union + `extra` arena) but deliberately kept two Ryo conventions instead of Zig's: (a) a parallel `spans: Vec<SimpleSpan>` per node, where Zig stores only `main_token` per node, retains the token stream in the tree, and derives source ranges on demand via `firstToken`/`lastToken` walks; (b) `NodeRef(NonZeroU32)` with a reserved slot-0 sentinel, where Zig uses `nodes[0]` as the root and `u32::MAX` as the none value. Both choices match the in-tree UIR/TIR conventions and kept the ariadne diagnostic pipeline untouched, but they cost 16 B per node and mean the AST is not a full Zig clone.
**Resolution:** Revisit if per-node memory matters at scale or if token-derived spans would simplify diagnostics: retain the lexer's token stream in `Ast`, add `main_token` to `Node`, port `firstToken`/`lastToken` for Ryo's `NodeTag` set, and drop the `spans` array. The index-convention half is likely permanent — `NonZeroU32` niche-filling is the Rust-idiomatic equivalent of Zig's `maxInt` none and matches UIR/TIR.
