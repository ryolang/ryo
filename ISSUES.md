...
**Files:** `src/ownership.rs` (Call arm of `visit_expr`), `src/codegen.rs` (`emit_frees`)
...
**Files:** `src/ownership.rs` (`rebind_to_init`, anonymous-temp Free pass in `analyze_function`)
...
**Files:** `src/ownership.rs` (`analyze_if_stmt`, `analyze_while_loop`, `analyze_for_range`)
...
### I-062 — Codegen's `free_schedule` is scanned linearly per statement (O(K·S) per function)
**Files:** `src/codegen.rs` (`emit_frees_matching` callers — `emit_due_frees`, `sweep_due_frees`)
**Summary:** Both `emit_due_frees` (called per `eval_inst` / `eval_inst_str`, plus on every `break`/`continue`) and `sweep_due_frees` (called after every statement) walk the full `sidecar.free_schedule` Vec. For a function with K free-points and S statements, total work is O(K · S) — quadratic in function size. Today's programs are small enough that the constant doesn't show, but K grows with every str-using statement and a long function will compound badly. The `pending: Vec` allocation per call is also a per-statement allocation cost.
**Resolution:** Build `free_by_anchor: HashMap<TirRef, SmallVec<[usize; 1]>>` once at function entry; `emit_due_frees` becomes O(1) by anchor lookup. `sweep_due_frees` is harder — track an unfired-cursor or invert into "frees ready to fire when their `after`/`target` is materialised." Combine with a top-level `free_schedule.is_empty()` short-circuit so functions with no scheduled Frees pay nothing.

### I-063 — ASan / Valgrind smoke harnesses duplicate fixtures, helpers, and Zig discovery
**Files:** `tests/asan_smoke.rs`, `tests/valgrind_smoke.rs`, `src/toolchain.rs` (`ensure_zig`)
**Summary:** Both smoke harnesses carry byte-for-byte copies of `runtime_lib_path()`, a test-local `zig_path()`, and the build-and-link scaffolding (`ryo build` → `zig cc` with `-fsanitize=address` or not → assert messages → conditional `-lunwind` on Linux). Five fixture programs (`simple_hello`, `mut_reassign`, `break_inside_loop_owner_after_read`, `pre_loop_owner_last_use_inside_loop`) are pasted into both files as Ryo source strings. Worse: the test-local `zig_path()` scans for the lexicographically-largest `zig-*` dir instead of honoring `ZIG_VERSION`, so it can pick a stale local install while the linker (which uses `toolchain::ensure_zig()`) picks the pinned version — outright correctness drift between test and prod paths.
**Resolution:** Promote `toolchain::ensure_zig()` to `pub(crate)` (or expose via a dedicated test-helper accessor) and add a `tests/common/mod.rs` (or `tests/common/smoke.rs`) module exposing `build_and_link(source, name, extra_link_args: &[&str])` plus a single `RYO_FIXTURES: &[(&str, &str)]` table. Both harnesses iterate the table; bug fixes to a fixture (or the build process) propagate to both modes.
...
**Summary:** M8.1c's break/continue pass schedules Frees only for *pre-loop* owners (those with `TirRef.raw() < body_min`). Owners declared inside the loop body whose only read is past a `break`/`continue` site never get a Free emitted on the jump path — the natural last-use Free (Task 3) is anchored at the read the jump skips, and the loop-exit pass deliberately excludes inside-loop owners. Verified post-M8.1c by direct inspection of `free_schedule` and arm64 disassembly for `while …: s = int_to_str(0); if cond: break; print(s)` — exactly one `ryo_str_free` call is scheduled, in the no-break arm; on the break path `b.eq` jumps straight to the function epilogue, skipping the free. The first allocation leaks.

**Why no test catches it:** Both macOS ASan (no LSan) and the Linux Docker harness (`run_linux_tests.sh`, ASan + LSan active) miss this leak. Sanity-checked: a C program calling `ryo_str_alloc(1)` and never freeing it IS flagged by LSan in the Docker harness, so the LSan setup works. But the leak inside the Cranelift-generated `main` is invisible to LSan — almost certainly because Cranelift-emitted code is not ASan-instrumented (no `__asan_init` registration, no shadow-memory bookkeeping, no stack-root reporting), so LSan's pointer-scan misses allocations whose only live references are in Cranelift-emitted stack frames or registers.

**Resolution:** Two independent fixes are needed.
1. *Compiler fix (this issue):* Track the set of inside-loop owners that became `Valid` *before* each break/continue site reached but were not yet read. Schedule a Free at the jump for any such owner. Requires a forward pass over the loop body that walks Valid lifetimes per-statement (not just at function exit), which is a non-trivial refactor of the lattice computation. Practical scope: M8.4 or later.
2. *Test infrastructure fix (separate issue, file when work begins):* Make CI actually able to detect this leak shape. Two paths: (a) add ASan instrumentation to Cranelift-emitted code (probably requires Cranelift upstream work — see https://github.com/bytecodealliance/wasmtime), or (b) instrument the Ryo runtime with a balanced-alloc/free counter that asserts at process exit. Path (b) is much cheaper and catches the same leak shape without needing ASan.
