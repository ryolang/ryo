...
- Error recovery: every failure emits `Unreachable` with `error_type` and analysis continues; the driver prints partial TIR under `--emit=tir`.
...
17. **Type-system foundations before user types** (I-018, I-088, I-109, I-110, I-111). `TypeId` enum retry; sidecar keyed by `DeclId`; inst→function map; document/assert tree-TIR; lexer macro table. The lazy-Sema worklist driver has shipped (see pipeline_alignment.md); comptime/generics now gate on the evaluator and monomorphization work itself — per-function TIR arenas are already positioned.
