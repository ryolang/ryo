...
| [ryo-compiler-llm-instructions.md](ryo-compiler-llm-instructions.md) | Normative contributor rules (R1–R21) for AI coding agents: data-oriented design, Rust discipline, diagnostics, Cranelift discipline, testing | **Keep** — update as conventions evolve; source-of-truth chain: spec → slicing final spec → proposal-review-issues → these instructions |
...
| [ryo-slicing-and-memory-model-final-spec.md](ryo-slicing-and-memory-model-final-spec.md) | Slicing/views/memory-model decisions (D1–D11) as a design decision record + specification; amends the base spec, supersedes the slice-projections and binary-pattern-matching proposals | **Move to spec** section by section as the decisions are implemented — then delete |
...
| [concurrency_poc.md](concurrency_poc.md) | Pre-Phase-1 concurrency proof-of-concept gate results — all three platforms (macOS, Linux, Windows) pass; includes the Windows cross-build recipe (`x86_64-pc-windows-gnu` + mingw-w64) | **Keep as reference** for the concurrency milestone (v0.4) — then delete |
