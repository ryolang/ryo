...
- Checked arithmetic lowers unfused even though Ryo already emits the fusible shape: codegen branches (`brif`) directly on the `sadd_overflow` flag result with no intermediate SSA boolean, yet aarch64 still emits `adds` + `cset` + `tst` + `b.ne` (x64: `seto` + `test` + `jne`) where one flags-conditional branch (`b.vs`/`jo`) suffices. A controlled minimal-CLIF experiment (pinned Cranelift 0.135.2) showed even a raw `trapnz` on the flag result gets the same treatment — Cranelift materializes the overflow flag into a register inside the `*_overflow` lowering itself and has no flag-forwarding into any attached branch or trap. The fix is upstream Cranelift work, not a Ryo-side shape change. Tracked as I-165 in `ISSUES.md`.
...
- **Branch-site structure is first-order for walltime on branchy workloads.** Rust 1.92's `DestinationPropagation` MIR pass merges same-valued locals, which collapses a conditional's two distinct branch sites into one `csel` feeding a single branch — the predictor sees one entry with mixed history. Measured cost: ~30% CoreMark regression for Stitch, ~50% gain for wasmi 2.0 when forced back apart in source ([wasmi 2.0 postmortem](https://wasmi-labs.github.io/blog/posts/wasmi-v2.0/)). Two consequences for Ryo: (1) our surviving overflow guards pay exactly this unfused-branch cost (`cset`+`tst`+`b.ne` vs one `b.vs`, I-165) — but the fix is upstream Cranelift flag-forwarding, since Ryo already branches directly on the flag result (see the guard-lowering lesson above); (2) **the Rust baseline arms are compiled by that same toolchain** — all current tables use Rust 1.98.0, so "vs Rust" on branch-dense suites (collatz) may be pessimistic for reasons that have nothing to do with Ryo. When a benchmark moves the Rust toolchain across a release boundary, re-measure the baseline arm before reading the delta.
