...
- **P4.** A slice's lifetime ends at its last use; P2's restriction lifts at that point. Across branches the last use is per-path: a read in a sibling arm does not keep the slice live on this arm's path, while a read after the join keeps it live on every arm's path. Two conservative cases are deliberate: a slice that is never read has no last use, so its projection lives to scope end and P2's restriction holds from creation; and when the last read on an arm's path sits inside a loop the slice was created outside of, the per-arm refinement is skipped and the slice stays live to the join.
...
- **W0003 RedundantMaterialize.** A heuristic, warnings-only lint flags the two bad shapes: (a) a materialize call in an argument position the re-borrow or a view-accepting builtin (e.g. `print`, the `str_push` suffix) already serves, and (b) a bound materialize result that never escapes while its source is never mutated afterward (mutating the source later is a legitimate defensive copy and is not flagged). Conservative by design: when unsure, no warning.
