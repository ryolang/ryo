...
*   **Materialization:** `str(view)` produces an owned `str` copy of a `strview` (allocates + copies) — the escape hatch for returning, storing, or moving viewed data past its owner. The argument must be a `strview`; anything else, including an owned `str`, is a type error. Materialization is never implicit: `x: str = view` stays a type error, while `x: str = str(view)` is the explicit, legal form. Warning **W0003** (`RedundantMaterialize`) flags materializations the call-scoped re-borrow already covers, and copies that never escape while their source is never mutated — heuristically, and never as an error.
...
The default is `hosted`; `ryo build --profile=core` or `runtime = "core"` in `ryo.toml` opts out. One language, no dialect: source that uses only `core` capabilities compiles identically under both profiles. `core` targets Wasm, CLI tools/small binaries, and freestanding environments (no scheduler, `panic` = abort, traces off, allocator-provided). Stdlib layering mirrors Rust's `core`/`std` precedent, adapted: `core` is always available; `std` re-exports it and adds hosted facilities.
