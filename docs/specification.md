...
- `bytes`: Owned, heap-allocated, contiguous byte buffer — the binary sibling of `str`. Move semantics, mutability by binding. Literal: `b"\x00\x01"`. Slicing yields a `bytesview` projection (§4.4 rules; no UTF-8 hazard, so scalar indexing `b[i]` **is** allowed, yielding `u8`). Bridging: `raw.to_str() -> Utf8Error!str` (UTF-8 validated), `text.to_bytes() -> bytes` (owned copy), `text.as_bytes() -> bytesview` (zero-copy view of the UTF-8 bytes — a §4.4 projection of the string, not a conversion). Warning **W0004** (`RedundantToBytes`) flags `to_bytes()` copies that are never mutated and never escape — provably replaceable by `as_bytes()` — heuristically, and never as an error. *(Rationale: `list[u8]` was the only — awkward — option for protocol/binary data; `bytes` fills it with the same ownership story as `str`.)*
