...
The default is `hosted`; `ryo build --profile=core` or `runtime = "core"` in `ryo.toml` opts out. One language, no dialect: source that uses only `core` capabilities compiles identically under both profiles. `core` targets CLI tools/small binaries and freestanding environments (no scheduler, `panic` = abort, traces off, allocator-provided); a Wasm target is planned but not currently provided (§16). Stdlib layering mirrors Rust's `core`/`std` precedent, adapted: `core` is always available; `std` re-exports it and adds hosted facilities.
